...
export interface UseCompositeRootParameters {
  orientation?: Accessor<'horizontal' | 'vertical' | 'both' | undefined>;
  ...
  rootRef?: Accessor<Element | null | undefined>;
  /**
   ...
   */
  disabledIndices?: Accessor<number[] | undefined>;
  /**
   ...
   * @default []
   */
  modifierKeys?: Accessor<ModifierKey[] | undefined>;
}
...
export function useCompositeRoot(params: UseCompositeRootParameters) {
  ...
  let rootRef = null as HTMLElement | null;
  const mergedRef = useForkRef(rootRef, params.rootRef);

  const elementsRef: Array<HTMLDivElement | null> = [];
  ...
  const onHighlightedIndexChange = useEventCallback((index, shouldScrollIntoView = false) => {
    ...
    if (shouldScrollIntoView) {
      const newActiveItem = elementsRef[index];
      scrollIntoViewIfNeeded(
        rootRef,
        newActiveItem,
  ...
  createEffect(() => {
    ...
    if (elementsRef.includes(activeEl)) {
      const focusedItem = elementsRef[highlightedIndex()];
  ...
  const props: Accessor<HTMLProps> = createMemo(() => ({
    'aria-orientation':
      mergedParams.orientation() === 'both'
        ? undefined
        : (mergedParams.orientation() as HTMLProps['aria-orientation']),
    ref: mergedRef,
    onFocus(event) {
    ...
    onKeyDown(event) {
      ...
      const isRtl = mergedParams.direction() === 'rtl';
      ...
      const minIndex = getMinListIndex(elementsRef, mergedParams.disabledIndices?.());
      const maxIndex = getMaxListIndex(elementsRef, mergedParams.disabledIndices?.());

      if (isGrid()) {
        const sizes =
          mergedParams.itemSizes?.() ||
          Array.from({ length: elementsRef.length }, () => ({
            width: 1,
            height: 1,
          }));
        ...
        const minGridIndex = cellMap.findIndex(
          (index) =>
            index != null &&
            !isListIndexDisabled(elementsRef, index, mergedParams.disabledIndices?.()),
        );
        // last enabled index
        ...
        nextIndex = cellMap[
          getGridNavigatedIndex(
            cellMap.map((itemIndex) => (itemIndex ? elementsRef[itemIndex] : null)),
            {
              event,
