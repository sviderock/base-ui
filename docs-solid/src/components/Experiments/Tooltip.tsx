...
export function Tooltip(props: TooltipProps) {
  return (
    <BaseTooltip.Root>
      <BaseTooltip.Trigger render={(p) => props.children(p)} />
      <BaseTooltip.Portal>
        <BaseTooltip.Positioner sideOffset={10}>
          <BaseTooltip.Popup class={classes.popup}>
            <BaseTooltip.Arrow class={classes.arrow}>
              <ArrowSvg />
            </BaseTooltip.Arrow>
            {props.text}
          </BaseTooltip.Popup>
...
export interface TooltipProps {
  text: string;
  children: Component<Accessor<HTMLProps>>;
}
