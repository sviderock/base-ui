      ...
      <Table.Body>
        <For each={Object.keys(local.data)}>
          {(name) => {
            ...
            const CssVaribleDescription = createMdxComponent(cssVariable.description, {
            ...
            return (
              <Table.Row>
                ...
                <Table.Cell colSpan={2}>
                  <div class="hidden xs:contents">{CssVaribleDescription}</div>
                  <div class="contents xs:hidden">
                    <ReferenceTablePopover>{CssVaribleDescription}</ReferenceTablePopover>
                  </div>
                </Table.Cell>
              </Table.Row>
            );
