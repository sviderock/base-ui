...
import type { DemoFile, DemoVariant } from 'docs-solid/src/blocks/Demo';
import { highlighter } from 'docs-solid/src/syntax-highlighting';
...
export async function DemoLoader(componentProps: DemoLoaderProps) {
  const [local, props] = splitProps(componentProps, ['path', 'scope']);
  const variants = createAsync(() => loadDemo({ path: local.path, scope: local.scope }));
