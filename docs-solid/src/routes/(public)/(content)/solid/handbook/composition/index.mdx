...
<Subtitle>A guide to composing Base UI components with your own Solid components.</Subtitle>
<Meta
  name="description"
  content="A guide to composing Base UI components with your own Solid components."
/>
...
```tsx title="index.tsx"
// prettier-ignore
<Menu.Trigger render={(props) => <MyButton {...props()} size="md" />}>
  Open menu
</Menu.Trigger>
```
  ...
  <Menu.Root>
    ...
    <Menu.Portal>
      <Menu.Positioner>
        <Menu.Popup>
          {/* prettier-ignore */}
          <Menu.Item render={(props) => <a {...props()} href="base-ui.com" />}>
            Add to Library
          </Menu.Item>
        </Menu.Popup>
      </Menu.Positioner>
...
If you are working in an extremely performance-sensitive application, you might want to pass a function to the `render` prop instead of a Solid element.
